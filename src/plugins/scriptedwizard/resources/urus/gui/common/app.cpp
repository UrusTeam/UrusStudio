...
{
    [IF WXFRAME][CLASS_PREFIX]Frame* frame = new [CLASS_PREFIX]Frame(0L[IF NONE], _("Urus GUI Application Template")[ENDIF NONE]);
    [IF WINDOWS]frame->SetIcon(wxICON(aaaa)); // To Set App Icon[ENDIF WINDOWS]
    frame->Show();[ENDIF WXFRAME]
    [IF WXDIALOG][CLASS_PREFIX]Dialog* dlg = new [CLASS_PREFIX]Dialog(0L[IF NONE], _("Urus GUI Template")[ENDIF NONE]);
